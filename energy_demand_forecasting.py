...
Original file is located at
    https://colab.research.google.com/drive/1u87NpYibPzYltk8tETZX-BKbjXAMVp4D
...
# Load the data
data = pd.read_csv('/content/TimeSeries_TotalSolarGen_and_Load_IT_2016 (1).csv')
data.head()
...
# Load the data
df = pd.read_csv('/content/TimeSeries_TotalSolarGen_and_Load_IT_2016 (1).csv')
...
# Optional: resample to daily if the data is hourly
# If hourly, seasonality = 24 (1 day); if daily, seasonality = 7 (1 week)
...
plt.plot(data)
plt.title("Italy Load Over Time")
plt.xlabel("Time")
plt.ylabel("Load (MW)")
plt.grid(True)
plt.show()

# Train-test split (last 30 data points for testing)
train = data.iloc[:-30]
test = data.iloc[-30:]

# Fit SARIMA model (example parameters, tune these as needed)
model = SARIMAX(train,
                order=(1, 1, 1),
                seasonal_order=(1, 1, 1, 24),  # assuming hourly data, daily seasonality
...
plt.plot(test.index, forecast_mean, label='Forecasted Load', color='green')
plt.fill_between(test.index, conf_int.iloc[:, 0], conf_int.iloc[:, 1], color='k', alpha=0.2)
plt.title('SARIMA Forecast vs Actual Load')
plt.xlabel('Time')
plt.ylabel('Load')
plt.legend()
plt.grid(True)
